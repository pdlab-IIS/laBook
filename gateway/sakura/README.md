...
responses, HTML, mutations, arbitrary queries and errors are not stored in this
server cache. Browser caching follows the separate rules below.
Versioned static URLs use a 16-character content hash generated by Flask. They
are cached for seven days in both the browser (`private, immutable`) and Sakura.
Changing a file changes its URL. Unversioned static files use five minutes in
both caches; covers use seven days in the browser and one hour in Sakura.
The server cache remains bounded to 2 MiB per object and 64 MiB total.
Only a single bounded `v` query parameter is accepted for static caching.

Successful list and scanner HTML uses `private, max-age=300, must-revalidate`
and `Vary: Cookie, Accept`, so ordinary revisits can reuse the page while login
state changes select another cache entry. HTML is never stored in the shared
server asset cache. Book data still loads from uncached APIs. Edit forms,
user/loan pages, authentication pages, errors, and mutation responses remain
no-store. HTML changes may take up to five minutes to appear on a normal visit;
a reload refreshes the page. Cached images/assets can remain on the device
after logout; any network access still passes gateway authentication.
