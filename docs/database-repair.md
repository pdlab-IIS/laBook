...
2026-09-01にデータ管理者の承認後、本番DBへ適用した。owner 469件と孤立Loan 8件は退避テーブルへ保存され、本番の`foreign_key_check`は477件から0件になった。適用直前backupは`/home/pdlab/labook/backups/migration/library.db_20260901-234542-072798.db`である。
...
次の方針はデータ管理者が承認し、2026-09-01に適用済みである。
...
未返却Loan 2件はどちらもこの孤立Loan 8件に含まれ、359個のbackupにも対応Bookはなかった。データ管理者は次の選択肢のうち、Loan全列を退避して稼働テーブルから除外する方針を承認した。

- Loan全列を退避して稼働テーブルから除外する
- ISBNだけを持つplaceholder Bookを作り、未返却状態と貸出履歴を維持する

空文字94件は別の業務上の意味を持たないものとしてNULL化した。孤立Loanの対応書籍が後から判明した場合は、`LoansOrphanArchive`を基に別migrationで復元する。
