...
class FrontendSecurityTests(unittest.TestCase):
    def test_external_book_and_review_data_is_not_inserted_as_html(self):
        ...
        self.assertIn("spnSpd.querySelector('.utility-menu__status')", index_source)
        self.assertIn("status.textContent = 'LOCAL'", index_source)
        self.assertIn("status.textContent = 'REMOTE'", index_source)
